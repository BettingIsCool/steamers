...
def get_log(bookmakers: str):
    ...
    """
    return conn.query(f"SELECT starts, sport_name, league_name, runner_home, runner_away, selection, market, line, prev_odds, curr_odds, droppct, oddstobeat, book_odds, book_val, book_name, book_url, timestamp, id, bet_str, timestamp_utc, starts_utc, updated_ago FROM {TABLE_LOG} WHERE book_slug IN {bookmakers} WHERE TIMESTAMPDIFF(SECOND, timestamp_utc, NOW()) < 3600 ORDER BY timestamp_utc DESC").to_dict('records')
