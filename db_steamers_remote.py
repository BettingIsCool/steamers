...
def get_log(bookmakers: str):
    ...
    """
    return conn.query(f"SELECT starts, sport_name, league_name, runner_home, runner_away, selection, market, line, prev_odds, curr_odds, droppct, oddstobeat, book_odds, book_val, book_name, book_url, timestamp, id, bet_str, timestamp_utc, starts_utc, updated_ago, drop_str FROM {TABLE_LOG} WHERE book_slug IN {bookmakers} AND TIMESTAMPDIFF(SECOND, timestamp_utc, NOW()) < 360000 ORDER BY timestamp_utc DESC").to_dict('records')
...
def append_user(data: dict):
    ...
    with conn.session as session:
        session.execute(text(query), params=dict(username=data['username'], default_odds_display='Decimal', default_timezone='Europe/London', default_minval=0.050, default_minodds=1, default_maxodds=100.00, default_book1='bet365', default_book2='bwin', default_book3='unibet', default_book4='williamhill', default_book5='winamax.es'))
        session.commit()
