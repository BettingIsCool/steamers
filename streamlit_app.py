...
if st.session_state.session_id == toolkit.get_active_session(st.session_state.user_id):
    ...
    if selected_books != '()':
        ...
        if bets:
            ...
            bets_df = bets_df[['UPDATED', 'BET', 'ODDS', 'BOOK', 'OTB', 'VALUE', 'DROP', 'LINK', 'STARTS', 'SPORT', 'LEAGUE', 'RUNNER_HOME', 'RUNNER_AWAY']]

            styled_df = bets_df.style.set_properties(**{'color': 'gray'}, subset=['OTB', 'UPDATED', 'DROP', 'STARTS', 'SPORT', 'LEAGUE', 'RUNNER_HOME', 'RUNNER_AWAY']).applymap(toolkit.color_cells, subset=['VALUE']).format({'LINE': '{:g}'.format, 'PODDS': '{:,.3f}'.format, 'CODDS': '{:,.3f}'.format, 'ODDS': '{:,.3f}'.format, 'OTB': '{:,.3f}'.format, 'VALUE': '{:,.2%}'.format})
