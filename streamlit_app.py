...
if st.session_state.session_id == toolkit.get_active_session(st.session_state.user_id):
    ...
    st.session_state.default_tag = st.sidebar.number_input("Select minimum value threshold", min_value=0.025, max_value=1.00, value=0.05, step=0.005, format="%0.3f", value=st.session_state.default_minval, on_change=db.change_user_minval, args=(username, placeholder1), key='default_minval_key')
